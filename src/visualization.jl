...
function visualize(simulation_results, radiusparticle, velocityparticle)
  ...
  for i in 1:numberofcells
    totalenergy += localenergy(massdisk, d_vel[i][1])
  end
  totalenergy += localenergy(massparticle, velocityparticle)


  function color(vel)
    (localenergy(massdisk,vel)/totalenergy,1. - localenergy(massdisk,vel)/totalenergy, 0.1)
    #(0.5,0.5,0.5)
  end

  c[:set_color](color(d_vel[1][1]))
  circles = [c]
  ax[:add_patch](c)

  for k in 2:numberofcells
    c = patch.Circle(d_pos[k][1],radiusdisk)
    c[:set_color](color(d_vel[k][1]))
    push!(circles,c)
    ax[:add_patch](c)
  end
  ...
  ax_energy[:set_ylabel]("Energy of Disk")

  ax_energy[:set_xlim](0, numberofcells+1)
  ax_energy[:set_ylim](0, initialenergy/1.5)
  ...
  function animate(i)
    ...
    if k == 0
      ...
      for j in 1:numberofcells
        ...
        circles[j][:set_color](color(d_vel[j][k]))
      end
      ...
      energy_text[:set_text]("Total energy = $(e_text)")
